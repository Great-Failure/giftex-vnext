...
export async function cancelExchangeHandler(
  request: HttpRequest,
  ...
  try {
    ...
    assertTransition(organizer.exchange.status, 'cancelled')
    ...
    trackEvent(context, 'V2ExchangeCancelled', { requestId, exchangeId })
    ...
    trackError(context, error, { requestId, function: 'v2/cancelExchange', exchangeId })
    return internalErrorResponse('Failed to cancel exchange', requestId)
  }
}

app.http('v2CancelExchange', {
  methods: ['POST'],
  authLevel: 'anonymous',
  route: 'v2/exchanges/{exchangeId}/cancel',
  handler: cancelExchangeHandler,
})
